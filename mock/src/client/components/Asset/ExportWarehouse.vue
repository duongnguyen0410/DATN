<template>
  <div class="wrap-import-warehouse">
    <a-button icon="export" type="primary" @click="showModal">{{ $t("export_warehouse") }}</a-button>

    <a-modal v-model="visible" :title="$t('export_warehouse_modal')" @ok="handleSubmit">
      ...
      <a-form :form="form">
        <a-row>
          <a-col class="px-1" :span="6">&nbsp;</a-col>
          <a-col class="px-1" :span="6">{{ $t("name") }}</a-col>
          <a-col class="px-1" :span="6">{{ $t("count") }}</a-col>
          <a-col class="px-1" :span="5">{{ $t("unit") }}</a-col>
          <a-col class="px-1" :span="1">&nbsp;</a-col>
        </a-row>

        <a-row v-for="(k, index) in form.getFieldValue('keys')" :key="k">
          <a-col class="px-1" :span="6">{{ $t("common.asset") }} {{ index }}</a-col>
                ...
                v-decorator="[
                  `counts[${k}]`,
                  {
                    ...
                    rules: [
                      ...
                      {
                        validator: (rule, value, callback) => customValidatorCount(rule, value, callback, k),
                        message: $t('cannot_export_greater_maximum')
                      }
                    ]
                  }
                ]"
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          const { keys, names, counts } = values;
          let listItem = [];
          for (let i = 0; i < keys.length; i++) {
            listItem.push({ id: names[i], count: counts[i] });
          }
          if (!listItem.length) return;
          this.exportAssetsAPI(listItem);
        }
      });
    },
    showModal() {
    ...
    async exportAssetsAPI(payload) {
      let data = await this.exportAssets(payload);
      if (data.error) {
    ...
    customValidatorCount(rule, value, callback, index) {
      const actionAfterEffect = k => {
        const names = this.form.getFieldValue("names");
        const counts = this.form.getFieldValue("counts");

        if (!names || names.length <= k) return callback();
        let asset = this.listAssets.find(item => item.id == names[k]);
        if (counts[k] > asset.count) return callback(new Error());
        callback();
      };

      setTimeout(() => actionAfterEffect(index), 10);
    },
